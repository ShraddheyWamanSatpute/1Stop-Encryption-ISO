 ...
 */
async function cleanupAuthEvents(): Promise<number> {
  const now = Date.now();
  const cutoff = now - AUTH_EVENTS_RETENTION_MS;
  const snapshot = await db.ref('authEvents').orderByChild('expiresAt').endAt(cutoff).once('value');
...
async function cleanupAuditLogs(): Promise<number> {
  const companiesSnapshot = await db.ref('auditLogs').once('value');
  ...
  for (const companyId of Object.keys(companies)) {
    const logs = companies[companyId] as Record<string, { expiresAt?: number }>;
