...
async function sendGmailTestEmail(tokens: { access_token?: string; refresh_token?: string; expiry_date?: number }, recipientEmail: string, senderEmail: string) {
...
async function sendOutlookTestEmail(tokens: { access_token?: string; refresh_token?: string; expires_in?: number }, recipientEmail: string, senderEmail: string) {
  const subject = 'Test Email from 1Stop System';
  const body = `Hello,
