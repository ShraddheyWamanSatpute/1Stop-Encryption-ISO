...
import {
  fetchPersonalSettingsSecureCall,
  updatePersonalSettingsSecureCall,
} from './settingsSecureClient';
...
/**
 * Fetch user's personal settings - uses secure Function (server-side decryption)
 * @param uid User ID
...
/**
 * Update user's personal settings - uses secure Function (server-side encryption)
 * @param uid User ID
...
export const updateUserPersonalSettings = async (uid: string, personalSettings: Partial<PersonalSettings>): Promise<void> => {
  try {
    await updatePersonalSettingsSecureCall(uid, personalSettings as Record<string, unknown>);
  } catch (error) {
