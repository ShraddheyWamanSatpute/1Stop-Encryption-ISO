...
export class PayrollEngine {
  ...
  private updateYTD(
    ...
    grossPay: number,
    tax: { taxPaidYTD: number },
    ni: { employeeNIYTD: number; employerNIYTD: number },
    studentLoan: { plans: Array<{ plan: string; ytd: number }> },
    pension: { employeeYTD: number; employerYTD: number }
  ): EmployeeYTDData {
    return {
      ...
      studentLoanPlan1YTD: studentLoan.plans.find((p) => p.plan === 'plan1')?.ytd || currentYTD.studentLoanPlan1YTD,
      studentLoanPlan2YTD: studentLoan.plans.find((p) => p.plan === 'plan2')?.ytd || currentYTD.studentLoanPlan2YTD,
      studentLoanPlan4YTD: studentLoan.plans.find((p) => p.plan === 'plan4')?.ytd || currentYTD.studentLoanPlan4YTD,
      postgraduateLoanYTD: studentLoan.plans.find((p) => p.plan === 'postgraduate')?.ytd || currentYTD.postgraduateLoanYTD
    }
  }
