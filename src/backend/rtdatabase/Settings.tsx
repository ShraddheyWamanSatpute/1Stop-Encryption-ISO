...
export const resendEmailVerification = async (user: FirebaseUser): Promise<void> => {
  try {
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to resend verification email: ${errorMsg}`);
  }
};
 ...
 */
export const createUserProfileInDb = async (userProfile: Record<string, unknown>): Promise<void> => {
  try {
...
export const fetchPersonalSettings = async (uid: string): Promise<PersonalSettings> => {
  try {
    ...
    if (snapshot.exists()) {
      let personal = snapshot.val() as PersonalSettings;

      // Decrypt sensitive fields (bankDetails, niNumber, taxCode) if encryption is initialized
      if (sensitiveDataService.isInitialized()) {
        try {
          personal = await sensitiveDataService.decryptUserPersonalData(
            personal as unknown as Record<string, unknown>
          ) as unknown as PersonalSettings;
        } catch (decryptError) {
          console.error("[Settings] Failed to decrypt personal settings:", decryptError);
        }
      }
 ...
 * Encrypts sensitive fields (bankDetails, niNumber, taxCode) before storage
 * @param uid User ID
...
export const updatePersonalSettings = async (uid: string, personalSettings: Partial<PersonalSettings>): Promise<void> => {
  try {
    let dataToSave: Record<string, unknown> = { ...personalSettings };

    // Encrypt sensitive fields before storing if encryption service is initialized
    if (sensitiveDataService.isInitialized()) {
      try {
        dataToSave = await sensitiveDataService.encryptUserPersonalData(dataToSave);
      } catch (encryptError) {
        console.error("[Settings] Failed to encrypt personal settings:", encryptError);
        if (import.meta.env.PROD) {
          throw new Error("Encryption required in production. Failed to encrypt personal settings.");
        }
      }
    } else if (import.meta.env.PROD) {
      throw new Error("Encryption service not initialized. Cannot store personal settings in production.");
    }

    const personalRef = ref(db, `users/${uid}/settings/personal`);
    await update(personalRef, dataToSave);
  } catch (error) {
...
export const subscribeToSettings = (
  ...
  const personalListener = onValue(personalRef, async (snapshot) => {
    if (snapshot.exists()) {
      let personal = snapshot.val() as PersonalSettings;
      if (sensitiveDataService.isInitialized()) {
        try {
          ...
          console.error("[Settings] Failed to decrypt personal settings in subscription:", decryptError);
        }
      }
      currentSettings.personal = personal;
 ...
 */
export const fetchUserProfileFromDb = async (uid: string): Promise<Record<string, unknown> | null> => {
  try {
    const userRef = ref(db, `users/${uid}`);
    const snapshot = await get(userRef);
 ...
 * @param updates Profile updates
 */
export const updateUserProfileInDb = async (uid: string, updates: Record<string, unknown>): Promise<void> => {
  try {
    const userRef = ref(db, `users/${uid}`);
    const updateData = {
      ...updates,
