      ...
      const tables = Object.entries(snapshot.val())
        ...
        .map(([id, data]) => {
          ...
          return {
            ...
            order: (tableData.order as number) || 0, // Provide default order if missing
          } as Table
        })
    ...
    if (snapshot.exists()) {
      const types = Object.entries(snapshot.val()).map(([id, data]) => {
        if (typeof data === "string") {
          ...
          const bookingTypeData = data as Record<string, unknown>
          return {
            id,
            name: (bookingTypeData.name as string) || "Unnamed Type",
            description: (bookingTypeData.description as string) || "",
            color:
        ...
        const tablesData = Object.entries(snapshot.val())
          ...
          .map(([id, data]) => {
            ...
            return {
              id,
              ...tableData,
              order: (tableData.order as number) || 0,
            } as Table
          })
...
export const fetchStockProducts = async (companyId: string, siteId: string): Promise<Array<Record<string, unknown>>> => {
  if (!companyId || !siteId) return []
