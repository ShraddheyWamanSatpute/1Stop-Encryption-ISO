...
export const getCompanyChats = async (basePath: string): Promise<Chat[]> => {
  try {
    ...
    return Object.values(allChats).filter((c): c is Chat => c.type === "company")
  } catch (error) {
...
export const getDepartmentChats = async (basePath: string, departmentId: string): Promise<Chat[]> => {
  try {
    ...
    if (!snapshot.exists()) return []
    const allChats = snapshot.val() as Record<string, Chat>
    return Object.values(allChats).filter((c): c is Chat => c.departmentId === departmentId)
  } catch (error) {
...
export const getRoleChats = async (basePath: string, roleId: string): Promise<Chat[]> => {
  try {
    ...
    if (!snapshot.exists()) return []
    const allChats = snapshot.val() as Record<string, Chat>
    return Object.values(allChats).filter((c): c is Chat => c.roleId === roleId)
  } catch (error) {
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      for (const chatId in allChats) {
        ...
        if ((chat as Chat & { categoryId?: string }).categoryId === categoryId) {
...
export const subscribeToNotifications = (
  ...
  const handleNotifications = (snapshot: { exists: () => boolean; val: () => Record<string, ChatNotification> }) => {
    if (snapshot.exists()) {
