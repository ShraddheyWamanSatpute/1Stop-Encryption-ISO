        ...
        try {
          if (sensitiveDataService.isInitialized()) {
            const decrypted = await sensitiveDataService.decryptCompanyData(
              ...
              { fieldsToDecrypt: [...BANK_ACCOUNT_ENCRYPTED_FIELDS] }
            );
            decryptedBankAccounts.push(decrypted as unknown as BankAccount);
          } else {
            decryptedBankAccounts.push(bankAccount);
          }
        } catch (decryptError) {
          // If decryption fails, return the raw data (might not be encrypted)
          console.warn(`[Finance] Failed to decrypt bank account ${bankAccount.id}, using raw data`);
          decryptedBankAccounts.push(bankAccount);
        }
      }
...
export const createInvoice = async (basePath: string, invoice: Omit<Invoice, "id">): Promise<Invoice> => {
  try {
    const invoicesRef = ref(db, `${basePath}/invoices`)
...
export const updateInvoice = async (basePath: string, invoiceId: string, updates: Partial<Invoice>): Promise<void> => {
  try {
...
export const createBankAccount = async (basePath: string, bankAccount: Omit<BankAccount, "id">): Promise<BankAccount> => {
  try {
    ...
    // Encrypt sensitive bank account data before saving if encryption service is initialized
    // Bank accounts contain sensitive financial data (account numbers, sort codes, IBANs)
    if (sensitiveDataService.isInitialized()) {
      try {
        // Bank accounts have accountNumber at top level - use BANK_ACCOUNT_ENCRYPTED_FIELDS
        const encrypted = await sensitiveDataService.encryptCompanyData(
          newBankAccount as unknown as Record<string, unknown>,
          { fieldsToEncrypt: [...BANK_ACCOUNT_ENCRYPTED_FIELDS] }
        );
        newBankAccount = encrypted as unknown as typeof newBankAccount;
        console.log("[Finance] Bank account data encrypted before storage");
      } catch (encryptError) {
        console.error("[Finance] Failed to encrypt bank account data:", encryptError);
        // In production, fail to prevent unencrypted storage
        if (import.meta.env.PROD) {
          throw new Error("Encryption required in production. Failed to encrypt bank account data.");
        }
        console.warn("[Finance] WARNING: Storing bank account data without encryption");
      }
    } else {
      // In production, encryption must be initialized
      if (import.meta.env.PROD) {
        throw new Error("Encryption service not initialized. Cannot store bank account data in production.");
      }
      console.warn("[Finance] Encryption service not initialized - storing bank account data without encryption");
    }
...
export const updateBankAccount = async (basePath: string, bankAccountId: string, updates: Partial<BankAccount>): Promise<void> => {
  try {
    ...
    if (sensitiveDataService.isInitialized()) {
      try {
        updatedFields = await sensitiveDataService.encryptCompanyData(updatedFields, {
          fieldsToEncrypt: [...BANK_ACCOUNT_ENCRYPTED_FIELDS]
        });
        console.log("[Finance] Bank account update data encrypted before storage");
      } catch (encryptError) {
        console.error("[Finance] Failed to encrypt bank account update data:", encryptError);
        // In production, fail to prevent unencrypted storage
        if (import.meta.env.PROD) {
          throw new Error("Encryption required in production. Failed to encrypt bank account update data.");
        }
        console.warn("[Finance] WARNING: Storing bank account update without encryption");
      }
    } else {
      // In production, encryption must be initialized
      if (import.meta.env.PROD) {
        throw new Error("Encryption service not initialized. Cannot update bank account data in production.");
      }
      console.warn("[Finance] Encryption service not initialized - storing bank account update without encryption");
    }
...
export const fetchReports = async (basePath: string): Promise<FinancialReport[]> => {
  try {
    const reportsRef = ref(db, `${basePath}/reports`)
    const snapshot = await get(reportsRef)
    const reports = snapshot.val() || {}
    return Object.values(reports) as FinancialReport[]
  } catch (error) {
