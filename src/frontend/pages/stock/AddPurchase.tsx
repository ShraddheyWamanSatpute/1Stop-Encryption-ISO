...
import type { Purchase, PurchaseItem, TabPanelProps } from "../../../backend/interfaces/Stock"
...
const AddPurchase: React.FC = () => {
  ...
  const [purchase, setPurchase] = useState<Purchase>({
    ...
    status: "Awaiting Submission", // Default to draft
    totalTax: 0,
    totalValue: 0,
    invoiceNumber: "",
    items: [],
      ...
      >
        <Box>
          ...
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">{id ? "Edit Purchase" : "New Purchase Order"}</Typography>
          </Breadcrumbs>
        </Box>
        <Button
          variant="outlined"
                      ...
                      >
                        ...
                        <TableCell>
                          <Autocomplete
                            ...
                            getOptionLabel={(option: any) => option.name || ""}
                            value={getAvailablePurchaseMeasures(item.itemID || item.productId).find((m) => m.id === item.measureId) || null}
                            onChange={(_, newValue) => {
                              const index = purchase.items.findIndex(
                                (i) => i.itemID === item.itemID && i.measureId === item.measureId,
                              )
                              handleMeasureChange(index, newValue?.id || "", newValue?.name || "Unknown Unit")
                            }}
