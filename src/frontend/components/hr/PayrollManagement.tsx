...
import { logDataExport } from "../../../backend/services/audit/dataExportAudit"
...
const PayrollManagement = () => {
  const { state: hrState, refreshEmployees, refreshPayrolls, refreshSchedules, addPayroll, updatePayrollRecord, deletePayrollRecord, fetchPayrollDetail } = useHR()
  ...
  const handleEditPayroll = async (record: Payroll) => {
    // Fetch full detail (secure list may only have non-sensitive fields)
    const hasFullData = typeof record.grossPay === "number" && typeof record.netPay === "number"
    const fullRecord = hasFullData ? record : (await fetchPayrollDetail(record.id)) ?? record
    setFormData({
      id: fullRecord.id,
      employeeId: fullRecord.employeeId,
      payPeriod: fullRecord.payPeriodStart,
      payPeriodStart: fullRecord.payPeriodStart,
      payPeriodEnd: fullRecord.payPeriodEnd,
      hoursWorked: fullRecord.hoursWorked,
      overtimeHours: fullRecord.overtimeHours,
      regularPay: fullRecord.regularPay,
      overtimePay: fullRecord.overtimePay,
      bonuses: fullRecord.bonuses || 0,
      grossPay: fullRecord.grossPay,
      totalGross: fullRecord.totalGross,
      deductions: {
        tax: fullRecord.deductions?.tax || fullRecord.taxDeductions || 0,
        insurance: fullRecord.deductions?.insurance || 0,
        retirement: fullRecord.deductions?.retirement || 0,
        other: fullRecord.deductions?.other || 0,
      },
      totalDeductions: fullRecord.totalDeductions,
      netPay: fullRecord.netPay,
      totalNet: fullRecord.totalNet,
      status: fullRecord.status,
      paymentMethod: fullRecord.paymentMethod || "direct_deposit",
      paymentDate: fullRecord.paymentDate || new Date().toISOString(),
      notes: fullRecord.notes || "",
    })
    setSelectedPayrollForCRUD(fullRecord)
    setEditMode(true)
    setOpenDialog(true)
  }
  ...
  const handleExportCSV = () => {
    ...
    if (companyId) {
      logDataExport(companyId, 'payroll', 'Exported payroll records to CSV', filteredPayrollRecords.length)
    }
  }
