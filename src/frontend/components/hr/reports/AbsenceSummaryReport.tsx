...
const AbsenceSummaryReport: React.FC = () => {
  ...
  const filteredAbsences = useMemo(() => {
    try {
      ...
      return safeArray(timeOffs).filter((absence: TimeOff) => {
        ...
        const employee = contextFilteredEmployees.find((e: Employee) => e.id === absence.employeeId)
  ...
  const metrics = useMemo(() => {
    try {
      ...
      const totalDays = filteredAbsences.reduce((sum: number, absence: TimeOff) => {
      ...
      const approved = filteredAbsences.filter((a: TimeOff) => safeString(a.status) === "approved").length
      const pending = filteredAbsences.filter((a: TimeOff) => safeString(a.status) === "pending").length
      const rejected = filteredAbsences.filter((a: TimeOff) => safeString(a.status) === "rejected").length
      
      const byType = filteredAbsences.reduce((acc: Record<string, AbsenceTypeMetrics>, absence: TimeOff) => {
  ...
  const groupedData = useMemo(() => {
    ...
    filteredAbsences.forEach((absence: TimeOff) => {
      let key = ""
      
      switch (groupBy) {
        case "employee": {
          const employee = employees.find((e: Employee) => e.id === absence.employeeId)
          key = employee ? `${employee.firstName} ${employee.lastName}` : "Unknown"
          break
        }
        case "department": {
          const emp = employees.find((e: Employee) => e.id === absence.employeeId)
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {Object.entries(metrics.byType).map(([type, data]: [string, AbsenceTypeMetrics]) => (
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          <TableBody>
            {filteredAbsences.slice(0, 50).map((absence: TimeOff) => {
              const employee = employees.find((e: Employee) => e.id === absence.employeeId)
