...
import type { Employee, Department, TimeOff } from "../../../../backend/interfaces/HRs"
import type { Site } from "../../../../backend/interfaces/Company"

interface ExtendedTimeOff extends TimeOff {
  certified?: boolean
  doctorNote?: boolean
...
const SicknessLogReport: React.FC = () => {
  ...
  const sickLeaves = useMemo(() => {
    try {
      ...
      const employeeIds = new Set(contextFilteredEmployees.map((e: Employee) => e.id))
      
      return safeArray(timeOffs)
        .filter((leave: TimeOff) => {
          if (!employeeIds.has(leave.employeeId)) return false
          const leaveType = safeString(leave.type)
          return leaveType === "sick_leave" || leaveType === "sickness" || leaveType === "sick"
        })
        .filter((leave: TimeOff) => {
          ...
          const employee = contextFilteredEmployees.find((e: Employee) => e.id === leave.employeeId)
          ...
          const extendedLeave = leave as ExtendedTimeOff
          const isCertified = extendedLeave.certified || extendedLeave.doctorNote || extendedLeave.medicalCertificate
      ...
      .map((leave: TimeOff) => {
        ...
        return {
          ...leave,
          isCertified: !!(extendedLeave.certified || extendedLeave.doctorNote || extendedLeave.medicalCertificate)
        } as ExtendedTimeOff
      })
    } catch (error) {
  ...
  const metrics = useMemo(() => {
    try {
      ...
      const totalDays = sickLeaves.reduce((sum: number, leave: ExtendedTimeOff) => {
      ...
      const certified = sickLeaves.filter((l: ExtendedTimeOff) => l.isCertified).length
      ...
      sickLeaves.forEach((leave: ExtendedTimeOff) => {
  ...
  const groupedData = useMemo(() => {
    ...
    sickLeaves.forEach((leave: ExtendedTimeOff) => {
      ...
      switch (groupBy) {
        case "employee": {
          const employee = employees.find((e: Employee) => e.id === leave.employeeId)
          key = employee ? `${employee.firstName} ${employee.lastName}` : "Unknown"
          break
        }
        case "department": {
          const emp = employees.find((e: Employee) => e.id === leave.employeeId)
          const dept = departments.find((d: Department) => d.id === emp?.departmentId)
    ...
    <Box>
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          <TableBody>
            {sickLeaves.slice(0, 50).map((leave: ExtendedTimeOff) => {
              const employee = employees.find((e: Employee) => e.id === leave.employeeId)
              const dept = departments.find((d: Department) => d.id === employee?.departmentId)
              const start = new Date(leave.startDate)
              const end = new Date(leave.endDate)
