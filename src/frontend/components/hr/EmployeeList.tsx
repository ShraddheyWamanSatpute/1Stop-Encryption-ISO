...
import { auditTrailService } from "../../../backend/services/gdpr/AuditTrailService"
import type { Employee } from "../../../backend/interfaces/HRs"
import EmployeeForm from "./EmployeeForm"
import ViewEmployee from "./ViewEmployee"
import CRUDModal from "../reusable/CRUDModal"
import EmployeeCRUDForm from "./forms/EmployeeCRUDForm"
...
interface ColumnConfig {
  ...
  format?: (value: unknown, employee: Employee) => string | React.ReactNode
}
...
const EmployeeList: React.FC = () => {
  ...
  const employeeFormRef = useRef<{ submit: () => void }>(null)
  ...
  }, [hrState, hrState?.employees?.length, hrState?.roles?.length, hrState?.departments?.length])
  ...
  const filteredEmployees = useMemo(() => {
    ...
    const filtered = hrState.employees.filter((employee) => {
      ...
      const employeeStatus = employee.status || (employeeWithRole.isActive === true ? "active" : undefined)
      ...
      const employeeDepartmentId = employee.departmentId || employeeWithRole.departmentID
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "hourlyRate":
          ...
          break
        default:
          aValue = String(a[sortBy as keyof Employee] || "")
          bValue = String(b[sortBy as keyof Employee] || "")
      }
  ...
  }, [hrState, hrState.employees, searchQuery, filterRole, filterStatus, filterDepartment, filterEmploymentType, sortBy, sortOrder])
  ...
  const getRoleName = useCallback((employee?: Employee) => {
    ...
    const employeeWithRole = employee as Employee & { roleID?: string }
    const roleId = employeeWithRole.roleID || employee.roleId
    ...
    if (!roleId) {
      console.log("getRoleName: No roleID found", { 
        employee: {
          ...
          roleID: employeeWithRole.roleID,
          role: employee.role,
  ...
  const handleViewEmployee = useCallback(async (employee: Employee) => {
    // Fetch full employee (decrypted) for view - list only has non-sensitive fields
    const full = await fetchEmployeeDetail(employee.id)
    setSelectedEmployeeForView((full ?? employee) as Employee)
    setViewEmployeeOpen(true)
  }, [fetchEmployeeDetail])

  const handleEditEmployee = useCallback(async (employee: Employee) => {
    // Fetch full employee (decrypted) for edit - list only has non-sensitive fields
    const full = await fetchEmployeeDetail(employee.id)
    handleOpenEmployeeCRUD(full ?? employee, 'edit')
  }, [handleOpenEmployeeCRUD, fetchEmployeeDetail])
  ...
  const filteredRoles = useMemo(() => {
    ...
    const filtered = hrState.roles.filter(role => {
      ...
      const roleWithDept = role as typeof role & { departmentID?: string; department?: string }
      const roleDeptId = role.departmentId || roleWithDept.departmentID || roleWithDept.department
      ...
      <DataHeader
        ...
        onExportCSV={() => {
          try {
            ...
            const rows = filteredEmployees.map(employee => {
              return visibleColumnConfigs.map(col => {
                switch (col.id) {
                  ...
                  case "address": {
                    const addr = employee.address as { street?: string; city?: string } | undefined
                    return addr?.street ? `${addr.street}, ${addr.city || ''}`.trim() : ""
                  }
                  case "actions":
                    return ""
                  default: {
                    const value = employee[col.id as keyof Employee]
            ...
            // Audit: log data export (ISO 27001 A.12, SOC 2 CC7)
            const uid = auth.currentUser?.uid
            const companyId = companyState.companyID
            if (uid && companyId) {
              auditTrailService.log('data_export', uid, companyId, {
                resourceType: 'employees',
                description: `Exported ${filteredEmployees.length} employees to CSV`,
                retentionPeriod: 24 * 30,
              }).catch(() => {})
            }
                        ...
                        switch (column.id) {
                          ...
                          default: {
                            ...
                            if (column.id === "address" && value && typeof value === "object") {
                              const addr = value as { street?: string; city?: string } | undefined
