...
interface EmployeeCRUDFormProps {
  ...
  onSave: (data: Omit<Employee, "id"> | Partial<Employee>) => void
}
...
const EmployeeCRUDForm = forwardRef<EmployeeCRUDFormRef, EmployeeCRUDFormProps>(({
  ...
  useImperativeHandle(ref, () => ({
    submit: () => {
      if (mode === 'create' && !dataConsentChecked) {
        alert('You must confirm that the employee has been informed about and consents to the Privacy Policy before creating their record.')
        return
      }
      ...
      return (onSave as (data: Omit<Employee, 'id'> | Partial<Employee>) => void | Promise<void>)(employeeData as Omit<Employee, 'id'>)
    }
  }), [formData, mode, dataConsentChecked, onSave])
  ...
  useEffect(() => {
    if (formData.roleId && hrState.roles && hrState.roles.length > 0) {
      ...
      if (selectedRole) {
        ...
        const roleWithDept = selectedRole as typeof selectedRole & { departmentID?: string; department?: string }
        const roleDeptId = selectedRole.departmentId || roleWithDept.departmentID || roleWithDept.department
        if (roleDeptId) {
    ...
    // formData.departmentId is intentionally excluded to prevent infinite loop
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    if (employee) {
      ...
      setFormData({
        ...
        emergencyContact: employee.emergencyContact ? {
          ...
          email: (employee.emergencyContact as typeof employee.emergencyContact & { email?: string }).email || ''
        } : {
          name: '',
          relationship: '',
          phone: '',
        ...
        dataConsentGiven: employee.dataConsentGiven || false,
        payslipEmailConsent: employee.payslipEmailConsent || false,
      })
      setDataConsentChecked(employee.dataConsentGiven || false)
                    ...
                    {mode === 'create' && (
                      <Grid item xs={12}>
                        ...
                        <Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 0.5, ml: 4 }}>
                          Required for processing employee data including payroll, tax, and HMRC submissions
                        </Typography>
                      </Grid>
                    )}
                    {(mode === 'edit' || mode === 'view') && (
                      <Grid item xs={12} sm={6}>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={formData.payslipEmailConsent}
                              onChange={(e) => handleChange('payslipEmailConsent', e.target.checked)}
                              disabled={isReadOnly}
                            />
                          }
                          label="Consent to email payslips"
                        />
                      </Grid>
                    )}
                  </Grid>
                      ...
                      onChange={(e) => {
                        ...
                        setFormData(prev => {
                          ...
                          if (prev.roleId && newDepartmentId) {
                            ...
                            if (currentRole) {
                              // Check both departmentId (primary) and department (fallback) fields
                              const roleWithDept = currentRole as typeof currentRole & { departmentID?: string; department?: string }
                              const roleDeptId = currentRole.departmentId || roleWithDept.departmentID || roleWithDept.department
