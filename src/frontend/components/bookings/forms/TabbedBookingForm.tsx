...
const TabbedBookingForm: React.FC<TabbedBookingFormProps> = ({
  ...
  const handleSendEmail = async (template?: any) => {
    try {
      ...
      if (!projectId) {
        throw new Error('Firebase project ID not configured. Set VITE_FIREBASE_PROJECT_ID in .env')
      }

      // Call the sendEmailWithGmail cloud function (requires auth for tenant verification)
      const region = 'us-central1'
      const fnBase = `https://${region}-${projectId}.cloudfunctions.net`
      const token = await auth.currentUser?.getIdToken()
      if (!token) throw new Error('You must be signed in to send emails')
