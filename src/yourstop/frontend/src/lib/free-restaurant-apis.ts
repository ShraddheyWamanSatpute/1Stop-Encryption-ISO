...
export class FreeRestaurantDataService {
        ...
        try {
          ...
          if (searchData.results && searchData.results.length > 0) {
            ...
            const detailedRestaurants = await Promise.all(
              searchData.results.slice(0, 20).map(async (place: { place_id: string }) => {
  ...
  private extractFoursquareCuisine(categories: Array<{ name?: string }>): string[] {
  ...
  private estimateRatingFromCategories(categories: Array<{ name?: string }>): number {
    // Higher-end categories get better estimated ratings
    const hasUpscale = Array.isArray(categories) && categories.some((cat) => 
  ...
  private extractSpecialties(categories: Array<{ name?: string }>): string[] {
  ...
  private extractFoursquareFeatures(categories: Array<{ name?: string }>): string[] {
  ...
  private parseGoogleOpeningHours(hours: { weekday_text?: string[] } | null | undefined): Record<string, { open: string; close: string; isClosed: boolean }> {
    if (!hours?.weekday_text) {
      return this.getDefaultHours();
    }

    const daysMap = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    const parsedHours: Record<string, { open: string; close: string; isClosed: boolean }> = {};
  ...
  private getDefaultHours(): Record<string, { open: string; close: string; isClosed: boolean }> {
    return {
  ...
  private parseGooglePhotos(photos: Array<{ photo_reference: string }>, apiKey: string): RestaurantPhoto[] {
    if (!photos) return [];
  ...
  private buildOSMAddress(tags: Record<string, string | undefined> | null | undefined): string {
    const parts = [
  ...
  private parseOSMOpeningHours(hours: string | null | undefined): Record<string, { open: string; close: string; isClosed: boolean }> {
  ...
  private extractOSMFeatures(tags: Record<string, string | undefined> | null | undefined): string[] {
    const features: string[] = [];
